        ...
        try {
            ...
            if ((loginchk.equals("") && !(loginchk.equals("ok"))) || (!(chk.equals("start"))) && loginData == null) {
                ...
                if (doLogin.getUserID() != 0 && doLogin.getDeleteFlag() != 1) {
                    ArrayList<ProductBeans> pdl = new ArrayList();
                    if (session.getAttribute("") != null) {
                        pdl = (ArrayList) session.getAttribute("");
                    }
                    if (session.getAttribute(doLogin.getName()) != null) {
                        ArrayList<ProductBeans> oldPdl = (ArrayList) session.getAttribute(doLogin.getName());
                        for (ProductBeans value : oldPdl) {
                            ProductBeans addObj = new ProductBeans();
                            addObj = value;
                            pdl.add(addObj);
                        }
                    }
                    session.setAttribute(doLogin.getName(), pdl);
